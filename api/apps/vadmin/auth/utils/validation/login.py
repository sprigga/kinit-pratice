...
class LoginForm(BaseModel):
    telephone: str
    password: str
    method: str = '0'  # 認證方式，0：密碼登錄，1：短信登錄，2：微信一键登錄
    platform: str = '0'  # 登錄平台，0：PC端管理系统，1：移動端管理系统

    # 重用驗證器：https://docs.pydantic.dev/dev-v2/usage/validators/#reuse-validators
...
class WXLoginForm(BaseModel):
    ...
    code: str
    method: str = '2'  # 認證方式，0：密碼登錄，1：短信登錄，2：微信一键登錄
    platform: str = '1'  # 登錄平台，0：PC端管理系统，1：移動端管理系统
        ...
        if not result.status:
            ...
            if not DEMO and count:
                ...
                if number >= DEFAULT_AUTH_ERROR_MAX_NUMBER:
                    await count.reset()
                    # 如果等于最大次數，那么就将用户 is_active=False
                    user.is_active = False
                    await db.flush()
        elif not user.is_active:
            self.result.msg = "此帳號已凍結無法登入！"
        elif data.platform in ["0", "1"] and not user.is_staff:
            self.result.msg = "此帳號無權限！"
        else:
            if not DEMO and count:
                await count.delete()
            self.result.msg = "OK"
