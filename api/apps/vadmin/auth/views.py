...
@app.delete("/users", summary="批量刪除用户", description="软刪除，刪除后清空所關聯的角色")
async def delete_users(ids: IdList = Depends(), auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.delete"]))):
    if auth.user.id in ids.ids:
        return ErrorResponse("不能刪除當前登錄用户")
    elif 1 in ids.ids:
        return ErrorResponse("不能刪除超级管理员用户")
...
@app.get("/users/{data_id}", summary="獲取用户信息")
async def get_user(
        data_id: int,
...
@app.post("/user/current/reset/password", summary="重置當前用户密碼")
...
@app.post("/user/current/update/info", summary="更新當前用户基本信息")
...
@app.post("/user/current/update/avatar", summary="更新當前用户头像")
...
@app.get("/user/admin/current/info", summary="獲取當前管理员信息")
...
@app.post("/user/export/query/list/to/excel", summary="导出用户查詢列表為excel")
async def post_user_export_query_list(
        header: list = Body(..., title="表头与對应字段"),
        params: UserParams = Depends(),
...
@app.get("/user/download/import/template", summary="下載最新批量导入用户模板")
...
@app.post("/users/init/password/send/sms", summary="初始化所選用户密碼並發送通知短信")
...
@app.post("/users/init/password/send/email", summary="初始化所選用户密碼並發送通知郵件")
...
@app.put("/users/wx/server/openid", summary="更新當前用户服務端微信平台openid")
...
@app.get("/roles/options", summary="獲取角色選擇項")
...
@app.get("/roles/{data_id}", summary="獲取角色信息")
async def get_role(
        data_id: int,
...
@app.get("/menus/tree/options", summary="獲取選單树選擇項，添加/修改選單時使用")
...
@app.get("/menus/role/tree/options", summary="獲取選單列表树信息，角色權限使用")
...
@app.delete("/menus", summary="批量刪除選單", description="硬刪除, 如果存在角色關聯则無法刪除")
...
@app.put("/menus/{data_id}", summary="更新選單信息")
async def put_menus(
        data_id: int,
...
@app.get("/menus/{data_id}", summary="獲取選單信息")
async def get_menus(
        data_id: int,
...
@app.get("/role/menus/tree/{role_id}", summary="獲取選單列表树信息以及角色選單權限ID，角色權限使用")
...
@app.get("/dept/tree/options", summary="獲取部門树選擇項，添加/修改部門時使用")
async def get_dept_options(auth: Auth = Depends(FullAdminAuth())):
    datas = await crud.DeptDal(auth.db).get_tree_list(mode=2)
    return SuccessResponse(datas)


@app.get("/dept/user/tree/options", summary="獲取部門树選擇項，添加/修改用户時使用")
...
@app.delete("/depts", summary="批量刪除部門", description="硬刪除, 如果存在用户關聯则無法刪除")
...
@app.put("/depts/{data_id}", summary="更新部門信息")
async def put_dept(
        data_id: int,
        data: schemas.Dept,
        auth: Auth = Depends(FullAdminAuth())
