...
class VadminIssueCategory(BaseModel):
    ...
    __table_args__ = ({'comment': '常见問题类别表'})

    name: Mapped[str] = mapped_column(String(50), index=True, nullable=False, comment="类别名稱")
...
class VadminIssue(BaseModel):
    ...
    __table_args__ = ({'comment': '常见問题记錄表'})
    ...
    title: Mapped[str] = mapped_column(String(255), index=True, nullable=False, comment="標题")
    content: Mapped[str] = mapped_column(Text, comment="内容")
    view_number: Mapped[int] = mapped_column(Integer, default=0, comment="查看次數")
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, comment="是否可见")
