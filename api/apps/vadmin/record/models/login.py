...
class VadminLoginRecord(BaseModel):
    ...
    __table_args__ = ({'comment': '登錄记錄表'})

    telephone: Mapped[str] = mapped_column(String(255), index=True, nullable=False, comment="帳號")
    status: Mapped[bool] = mapped_column(Boolean, default=True, comment="是否登錄成功")
    platform: Mapped[str] = mapped_column(String(8), comment="登陆平台")
    login_method: Mapped[str] = mapped_column(String(8), comment="認證方式")
    ...
    postal_code: Mapped[str | None] = mapped_column(String(255), comment="郵政编碼")
    ...
    request: Mapped[str | None] = mapped_column(Text, comment="請求信息")
