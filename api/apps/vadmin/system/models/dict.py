...
class VadminDictType(BaseModel):
    ...
    __table_args__ = ({'comment': '字典類型表'})

    dict_name: Mapped[str] = mapped_column(String(50), index=True, nullable=False, comment="字典名稱")
    dict_type: Mapped[str] = mapped_column(String(50), index=True, nullable=False, comment="字典類型")
    disabled: Mapped[bool] = mapped_column(Boolean, default=False, comment="字典状態，是否禁用")
...
class VadminDictDetails(BaseModel):
    ...
    __table_args__ = ({'comment': '字典詳情表'})

    label: Mapped[str] = mapped_column(String(50), index=True, nullable=False, comment="字典標籤")
