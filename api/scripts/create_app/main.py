...
class CreateApp:
    ...
    def run(self):
        """
        自動創建初始化 APP 结构，如何該路径已經存在，则不执行
        """
        if self.exist(self.app_path):
            print(f"{self.app_path} 已經存在，無法自動創建，請刪除後，重新執行。")
            return False
        print("開始生成 App 目錄：", self.path)
        path = []
        ...
        self.create_pag(os.path.join(self.app_path, "crud"))
        self.generate_file("views.py")
        # self.generate_file("crud.py")
        print("App 目錄生成结束", self.app_path)
