...
class ExcelManage:
    ...
    def open_sheet(
        ...
        """
        初始化 excel 文件
        :param sheet_name: 表单名稱，為空则默认第一个
        :param file:
        :param read_only:
        :param data_only:
        :return:
    ...
    def __set_row_style(
        ...
        :param alignment_style: 单元格内容的對齐设置
        :param font_style: 单元格内容的字体樣式设置
    ...
    def __auto_width(self):
        ...
        for row in self.sheet.rows:
            for cell in row:
                if cell.value:
                    # 遍历整个表格，把該列所有的单元格文本進行长度對比，找出最长的单元格
                    # 在對比单元格文本時需要将中文字符识别為1.7个长度，英文字符识别為1个，这里只需要将文本长度直接加上中文字符數量即可
                    # re.findall('([\u4e00-\u9fa5])', cell.value)能够识别大部分中文字符
                    cell_len = 0.7 * len(re.findall('([\u4e00-\u9fa5])', str(cell.value))) + len(str(cell.value))
                    dims[cell.column] = max((dims.get(cell.column, 0), cell_len))
        for col, value in dims.items():
            # 设置列宽，get_column_letter用于獲取數字列号對应的字母列号，最后值+2是用来調整最终效果的
