...
# @File           : aliyun.py
# @IDE            : PyCharm
# @desc           : 最新版阿里云短信服務發送程序（Python版本）2022-11-08

"""
短信 API 官方文檔：https://help.aliyun.com/document_detail/419298.html?spm=5176.25163407.help.dexternal.6ff2bb6ercg9x3
發送短信 官方文檔：https://help.aliyun.com/document_detail/419273.htm?spm=a2c4g.11186623.0.0.36855d7aRBSwtP
Python SDK 官方文檔：https://help.aliyun.com/document_detail/215764.html?spm=a2c4g.11186623.0.0.6a0c4198XsBJNW

环境要求
Python 3
安装 SDK 核心庫 OpenAPI ，使用pip安装包依赖:
...
class AliyunSMS(DBGetter):
    ...
    def __init__(self, telephones: List[str], rd: Redis = None):
        ...
        self.sign_conf = None  # 數據庫中 sms_sign_name_* 的配置
        self.template_code_conf = None  # 數據庫中 sms_template_code_* 的配置
        # 以上两个配置項的好处在于可以灵活配置短信信息，不需要改代碼
    ...
    async def _get_settings_async(self, retry: int = 3):
        ...
        """
        if not self.rd:
            raise ValueError("缺少 redis 對象參數！")
        elif not self.sign_conf or not self.template_code_conf:
            raise ValueError("缺少短信簽名信息和短信模板ID！")
    ...
    def _get_template_param(self, **kwargs) -> str:
        ...
        可以被子类继承的受保护的私有方法
        """
        raise NotImplementedError("該方法應該被重寫！")
    ...
    def _validation(self, telephone: str, resp: dysmsapi_20170525_models.SendSmsResponse) -> bool:
        ...
        if resp.body.code == "OK":
            logger.info(f'{send_time} {telephone} 短信發送成功，返回code：{resp.body.code}')
            return True
        else:
            logger.error(f'{send_time} {telephone} 短信發送失敗，返回code：{resp.body.code}，請参考文檔：{self.doc}')
            return False
    ...
    def get_code(length: int = 6, blend: bool = False) -> str:
        """
        随机獲取短信驗證碼
        短信驗證碼只支持數字，不支持字母及其他符号

        :param length: 驗證碼长度
        :param blend: 是否 字母+數字 混合
        """
        code = ""  # 創建字符串变量,存储生成的驗證碼
        for i in range(length):  # 通過for循环控制驗證碼位數
            num = random.randint(0, 9)  # 生成随机數字0-9
            if blend:  # 需要字母驗證碼,不用傳参,如果不需要字母的,关键字alpha=False
    ...
    def check_telephone_format(telephone: str) -> bool:
        ...
        if not telephone:
            raise CustomException(msg=f"帳號碼（{telephone}）不能為空", code=400)
        elif not re.match(REGEX_TELEPHONE, telephone):
            raise CustomException(msg=f"帳號碼（{telephone}）格式不正確", code=400)
        return True
