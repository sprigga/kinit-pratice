...
class WXOAuth:
    ...
    async def get_code2session(self, code: str) -> dict:
        ...
        if "openid" not in result:
            logger.error(f"微信校驗失敗：{result}, code：{code}")
        else:
            logger.info(f"微信校驗成功：{result}, code：{code}")
        return result
    ...
    async def get_phone_number(self, code: str):
        ...
        官方文檔：https://developers.weixin.qq.com/miniprogram/dev/api-backend/open-api/phonenumber/phonenumber.getPhoneNumber.html
        :param code: 動態令牌。可通過動態令牌换取用户帳號。
        ...
        失敗：{'errcode': 40001, 'errmsg': 'invalid credential, access_token is invalid or not latest rid: 62690257-2894b530-58c6fcf3'}
        """
        if not self.appid or not self.secret:
            await self.__get_settings()
        ...
        if not access_token.get("status", False):
            result = {'errcode': 40001, 'errmsg': '獲取微信令牌失敗'}
            # print(result)
            logger.error(f"獲取微信用户帳號失敗：{result}")
            return result
        params = {
        ...
        if result.get("errcode", 0) == 0:
            # print("獲取微信用户帳號成功", result)
            logger.info(f"獲取微信用户帳號失敗：{result}, code：{code}")
        else:
            # print("獲取微信用户帳號失敗", result)
            logger.error(f"獲取微信用户帳號失敗：{result}, code：{code}")
            if result.get("errcode", 0) == 40001:
                await at.update()
                if self.retry_count > 0:
                    logger.error(f"重試獲取微信帳號，重試剩餘次數, {self.retry_count}")
                    self.retry_count -= 1
