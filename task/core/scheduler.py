...
from .listener import before_job_execution, after_job_execution
...
class Scheduler:
    ...
    def start(self, listener: bool = True) -> None:
        ...
        if listener:
            # 注册事件监听器
            self.scheduler.add_listener(before_job_execution, EVENT_JOB_SUBMITTED)
            self.scheduler.add_listener(after_job_execution, EVENT_JOB_EXECUTED)
    ...
    def add_job(
        ...
        """
        date触发器用于在指定的日期和时間触发一次任务。它适用于需要在特定时間点执行一次的任务，例如执行一次备份操作。
        :param job_class: 类路径
        :param trigger: 触发條件
        :param name: 任务名称
        :return:
        ...
        if job_class:
            ...
            raise ValueError(f"添加任务失败，未找到該模块下的方法：{job_class}")
    ...
    def add_cron_job(
        ...
        通过 cron 表达式添加定时任务
        :param job_class: 类路径
        :param expression: cron 表达式，六位或七位，分别表示秒、分鐘、小时、天、月、星期几、年
        :param start_date: 触发器的开始日期时間。可选参数，默认为 None。
        :param end_date: 触发器的结束日期时間。可选参数，默认为 None。
    ...
    def add_interval_job(
        ...
        :param end_date: 表示任务的结束时間，可以设置为 datetime 对象或者字符串。
                         例如，设置 end_date='2023-06-23 10:00:00' 表示任务在 2023 年 6 月 23 日 10 点结束。
        :param start_date: 表示任务的起始时間，可以设置为 datetime 对象或者字符串。
                           例如，设置 start_date='2023-06-22 10:00:00' 表示从 2023 年 6 月 22 日 10 点开始执行任务。
        :param timezone：表示时区，可以设置为字符串或 pytz.timezone 对象。例如，设置 timezone='Asia/Taipei' 表示使用上海时区。
        :param jitter：表示时間抖動，可以设置为整数或浮点数。例如，设置 jitter=2 表示任务的执行时間会在原定时間上随机增加 0~2 秒的时間抖動。
        :param name: 任务名称
        :param args: 非关键字参数
        :return:
        """
    ...
    def __import_module(self, expression: str):
        ...
        try:
            ...
            raise ValueError(f"未找到該模块：{module_pag}")
        except AttributeError:
            raise ValueError(f"未找到該模块下的方法：{module_class}")
    ...
    def __parse_cron_expression(expression: str) -> tuple:
        """
        解析 cron 表达式
        :param expression: cron 表达式，支持六位或七位，分别表示秒、分鐘、小时、天、月、星期几、年
        :return: 解析后的秒、分鐘、小时、天、月、星期几、年字段的元组
        """
