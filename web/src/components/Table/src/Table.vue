...
export default defineComponent({
  ...
  props: {
    ...
    // 是否所有的超出隱藏，优先级低于schema中的showOverflowTooltip,
    ...
    // 仅對 type=selection 的列有效，類型為 Boolean，為 true 则會在數據更新之後保留之前選中的數據（需指定 row-key）
    ...
    // row-class-name, 類型為 (row: Recordable, rowIndex: number) => string | string
    rowClassName: {
    ...
    emptyText: propTypes.string.def('暂無數據'),
    // 表格工具欄緩存唯一標識符
