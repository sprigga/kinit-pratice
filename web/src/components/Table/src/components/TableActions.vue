...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 獲取 table columns 是否已经存在序號列，如果存在则更新初始狀態，如果不存在则添加
    ...
    if (numberColumnField === undefined) {
      ...
      if (tableColumns.value[0].type === 'selection') {
        ...
        tableColumns.value.unshift(indexColumn)
      }
    } else {
    ...
    if (cacheTableHeadersKey) {
      // 獲取緩存中的 table columns 狀態並覆盖到當前的 table columns 中
                      ...
                      <ElScrollbar max-height="400px">
                        ...
                        >
                          {tableColumns.value.map((element) => {
                            if (element.type === 'index' || element.type === 'selection')
                              return <span></span>
                            return (
