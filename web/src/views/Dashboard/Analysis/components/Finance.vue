...
<template>
  ...
  <ElRow :gutter="20" class="pt-4">
    <ElCol :xs="12" :sm="6" class="border-r-1 border-r-[#f0f0f0] b-r-solid mb-2">
      <ElSkeleton :loading="loading" animated :rows="4">
        <div>
          <div class="text-[#787a7d] text-[12px]">銷售額</div>
    ...
    <ElCol :xs="12" :sm="6" class="border-r-1 border-r-[#f0f0f0] b-r-solid mb-2">
      <ElSkeleton :loading="loading" animated :rows="4">
        <div>
          <div class="text-[#787a7d] text-[12px]">充值金額</div>
          <div class="text-[#121315] text-[20px] mt-[10px]">{{ formatMoney(899999.0) }}</div>
        </div>
      </ElSkeleton>
    </ElCol>
    <ElCol :xs="12" :sm="6" class="border-r-1 border-r-[#f0f0f0] b-r-solid mb-2">
      <ElSkeleton :loading="loading" animated :rows="4">
        <div>
          <div class="text-[#787a7d] text-[12px]">銷量</div>
    ...
    <ElCol :xs="12" :sm="6" class="border-r-1 border-r-[#f0f0f0] b-r-solid">
      <ElSkeleton :loading="loading" animated :rows="4">
        <div>
          <div class="text-[#787a7d] text-[12px]">訂單單數</div>
  ...
  <div class="mt-8">
    ...
    >
      ...
      <ElTableColumn prop="name" label="商品名稱" />
      <ElTableColumn prop="category" label="商品品類" />
      <ElTableColumn prop="order_quantity" label="商品購買次數" />
      <ElTableColumn prop="buy_user_number" label="商品購買人數" />
      <ElTableColumn prop="sales" label="商品銷售額" align="center">
        <template #default="{ row }">
          <span>{{ formatMoney(row.sales) }}</span>
        </template>
      </ElTableColumn>
      <ElTableColumn prop="sales_volume" label="商品銷量" align="center" />
    </ElTable>
  </div>
</template>
