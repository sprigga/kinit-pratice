...
const sendPasswordToSMS = async () => {
  ...
  if (selections.value.length > 0) {
    dialogTitle.value = '重置密碼並發送短信'
    actionType.value = 'sms'
    currentRow.value = undefined
    dialogVisible.value = true
  } else {
    return ElMessage.warning('請先選擇數據')
  }
}
      ...
      <template #toolbar>
        <ElRow :gutter="10">
          ...
          <ElCol :span="1.5" v-hasPermi="['auth.user.import']">
            <BaseButton @click="importList">批量導入用户</BaseButton>
          </ElCol>
          <ElCol :span="1.5" v-hasPermi="['auth.user.export']">
            <BaseButton @click="exportQueryList()">導出篩選用户</BaseButton>
          </ElCol>
          <ElCol :span="1.5" v-hasPermi="['auth.user.reset']">
            <BaseButton @click="sendPasswordToEmail">重置密碼通知郵件</BaseButton>
          </ElCol>
          <ElCol :span="1.5" v-hasPermi="['auth.user.delete']">
            <BaseButton type="danger" @click="delData(null)">批量刪除</BaseButton>
          </ElCol>
        </ElRow>
      </template>
    </Table>
