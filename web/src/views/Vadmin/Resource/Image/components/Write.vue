          ...
          return (
            <>
              <div class="flex justify-between w-[100%]">
                <span>圖片資源</span>
                <span>
                  最大數量限制：{fileList.value.length}/{maxLimit.value}
                </span>
              </div>
              <ElUpload
              ...
              >
                {fileList.value.length < maxLimit.value ? (
                  <div class="resource-image-uploader-icon">
                    ...
                    <span class="text-[12px]">點擊或拖拽到這裡</span>
...
const beforeImageUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  if (!isIMAGE) {
    ElMessage.error('上傳圖片素材必須是 JPG/PNG/ 格式!')
  }
  if (!isLtSize) {
    ElMessage.error('上傳圖片素材大小不能超過 3MB!')
  }
...
const handleExceedLimit = () => {
  ElMessage.error('上傳失敗，超出圖片最大數量限制!')
}
