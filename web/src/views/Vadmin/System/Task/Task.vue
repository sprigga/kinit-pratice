...
const runOnceTask = async (row: any) => {
  ElMessageBox.confirm('是否確認立即執行一次任務', '顯示', {
    ...
    if (res) {
      if (res.data > 0) {
        ElMessage.success('任務成功被主機接收！')
      } else {
        ElMessage.error('執行失敗，未有主機接收任務，請檢查定時任務序狀態！')
      }
    }
  })
}
  ...
  <ContentWrap>
    ...
    >
      <template #toolbar>
        <ElRow :gutter="10">
          <ElCol :span="1.5">
            <BaseButton type="primary" @click="addAction">新增定時任務</BaseButton>
          </ElCol>
          <ElCol :span="1.5">
            <BaseButton type="primary" @click="toRecord(null)">調度日誌</BaseButton>
          </ElCol>
          <ElCol :span="1.5">
            <BaseButton type="primary" @click="generateCronExpression"
              >快速生成 Cron 表達式</BaseButton
            >
          </ElCol>
        </ElRow>
      </template>
