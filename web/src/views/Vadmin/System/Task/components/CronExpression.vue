...
// Cron 表達式 轉 表達式字段
const cronToField = () => {
  const cronExpression = tableData.value[1].seconds // 獲取 Cron 表達式
...
// cron 表達式轉中文描述：https://github.com/bradymholt/cRonstrue
// cronjs 解析器與匹配器(五位)：https://github.com/datasert/cronjs
// cron 表達式驗證器：https://github.com/Airfooox/cron-validate
const validate = () => {
  const cronExpression = tableData.value[1].seconds // 獲取 Cron 表達式
...
<template>
  <div class="cron-expression-box">
          ...
          <div class="list-item mt-4">
            <ElRadio label="2">
              <ElInput v-model="seconds2_1" @change="secondsChange" class="!w-[200px]">
                <template #prepend> 從 </template>
              </ElInput>
              <ElInput v-model="seconds2_2" @change="secondsChange" class="!w-[400px]">
                <template #prepend> 秒開始，每 </template>
                <template #append>秒執行一次</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
          ...
          <div class="list-item mt-4">
            <ElRadio label="2">
              ...
              <ElInput v-model="minutes2_2" @change="minutesChange" class="!w-[400px]">
                <template #prepend> 分開始，每 </template>
                <template #append>分執行一次</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
          ...
          <div class="list-item mt-4">
            <ElRadio label="2">
              ...
              <ElInput v-model="hour2_2" @change="hourChange" class="!w-[400px]">
                <template #prepend> 小時開始，每 </template>
                <template #append>小時執行一次</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
          ...
          <div class="list-item mt-4">
            <ElRadio label="4">
              ...
              <ElInput v-model="day2_2" @change="dayChange" class="!w-[400px]">
                <template #prepend> 日開始，每 </template>
                <template #append>日執行一次</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-4">
            <ElRadio label="5">
              <ElInput v-model="day3_1" @change="dayChange" class="!w-[600px]">
                ...
                <template #append>號最近的那個工作日</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
          ...
          <div class="list-item mt-4">
            <ElRadio label="2">
              ...
              <ElInput v-model="month2_2" @change="monthChange" class="!w-[400px]">
                <template #prepend> 月開始，每 </template>
                <template #append>月執行一次</template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
          ...
          <div class="list-item mt-2">
            <ElRadio label="2">
              <ElInput v-model="week1_1" @change="weekChange" class="!w-[300px]">
                <template #prepend> 周期從星期 </template>
              </ElInput>
          ...
          <div class="list-item mt-4">
            <ElRadio label="4">
              <ElInput v-model="week3_1" @change="weekChange" class="!w-[600px]">
                <template #prepend> 本月最後一個星期 </template>
              </ElInput>
            </ElRadio>
          </div>
          <div class="list-item mt-2">
    ...
    <div class="mt-5">
      <span class="text-[17px]">生成表達式</span>
      <ElTable
        :data="tableData"
        style="width: 100%"
      ...
      >
        ...
        <ElTableColumn prop="hour" label="小時" align="center">
          <template #default="{ row, $index }">
      ...
      <div class="mt-3 text-center">
        <BaseButton type="primary" @click="getRunDatetime">獲取最近十次運行時間</BaseButton>
        <BaseButton type="primary" @click="validate">Cron 表達式驗證</BaseButton>
        <BaseButton type="primary" @click="getCronExample">Cron 表達式示例</BaseButton>
      </div>
    </div>
  </div>
  ...
  <Dialog v-model="dialogVisible2" title="Cron 表達式示例" width="700px" height="620px" top="12vh">
    <CronExample />
  </Dialog>
</template>
